...
powerOn c oP = set (contextDBus c) oP "org.bluez.Adapter1" "Powered" True

powerOff :: Context -> ObjectPath -> IO (Maybe ())
powerOff c oP = set (contextDBus c) oP "org.bluez.Adapter1" "Powered" False
...
removeDevice c oP device = do
  let mc = (methodCall oP ("org.freedesktop.Adapter1") ("RemoveDevice")) {
    methodCallDestination = Just "org.bluez",
